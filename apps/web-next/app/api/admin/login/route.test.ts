...
describe("POST /api/admin/login", () => {
  ...
  it("rejects requests with disallowed Origin", async () => {
    const res = await POST(makeRequest({ email: "a@b.co", password: "x" }, "https://evil.com"));
  ...
  it("returns 401 when password mismatch", async () => {
    mockUsers.push({ id: "u1", email: "a@b.co", passwordHash: "$hash" });
    bcryptCompareImpl = async () => false;
    const res = await POST(makeRequest({ email: "a@b.co", password: "wrong" }));
    expect(res.status).toBe(401);
  });
  ...
  it("sets admin_session cookie and returns 200 on valid credentials", async () => {
    ...
    const res = await POST(makeRequest({ email: "a@b.co", password: "right" }));
