...
- **Fixed 2026-09-23 on `feat/professional-case-studies`** (professional case-studies PR; strike this item in the parked-backlog table on `chore/followup-parking` once merged). Root cause was not the section: `app/[locale]/template.tsx` wrapped every page in a framer element with a permanent `animate="visible"`. Descendant motion elements inherit that label as variant context at mount, so framer records them as already "visible"; when `useReducedMotionSafe` re-syncs to `true` after hydration and the reveal helpers switch to `initial: false`, no target is ever set (even `animate: "visible"` is a no-op, the recorded value being unchanged), so the server-rendered `opacity:0` never clears — for every reveal element on the home, not only the cards. Fix: the template passes `animate` only while its entry animation plays (`animateEntry ? "visible" : undefined`), and `getInViewReveal`/`getMountReveal` return `{ initial: false, animate: "visible" }` under reduced motion. Verified on the production build with Chromium `reducedMotion: "reduce"`: 804 elements under `section` on `/en`, 0 at opacity 0 (Bookable showcase frames and `aria-hidden` excluded); covered by `e2e/docs/case-studies.spec.ts` (desktop + mobile).
